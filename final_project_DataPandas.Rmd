...
Okay, Let us now test using other ML classification models like Logistic Regression, Random Forest Classifier, Decision Tree classifier, KNN Classifier.
...
train_rf <- student_depression_dataset[split_index, ]
test_rf  <- student_depression_dataset[-split_index, ]
...
rf_model <- randomForest(Depression ~ ., data = train_rf, ntree = 100)

rf_predictions <- predict(rf_model, newdata = test_rf)

rf_conf_matrix <- confusionMatrix(rf_predictions, test_rf$Depression)
...
train_scaled <- as.data.frame(scale(train_knn[, -which(names(train_knn) == "Depression")]))
test_scaled  <- as.data.frame(scale(test_knn[, -which(names(test_knn) == "Depression")]))

train_scaled$Depression <- train_knn$Depression
test_scaled$Depression  <- test_knn$Depression
...
Let's see the feature importance of this model since it has the highest accuracy.
